def display(tab):
    tab.write("This tutorial explains how to fine tune a molecule base model for predicting Egc for polymers. In this section we will fine tune all the layers of the base model. This model is referred to as A0B3 in the project. The fingerprints used here are polyBERT fingerprints. ")
    tab.write("You can find the running tutorial on google colab: https://colab.research.google.com/drive/1fqtufiJAsC_DJTsqXSuKfuaJ7xm_RR1K?usp=sharing")
    ...
    code_5 = """
    state_dict = torch.load("molecule_polyBERT.pth", map_location=torch.device('cpu'))
    """
    ...
    #Loading the base_model weights and biases
    net.load_state_dict(state_dict, strict=False)
    net = net.to(DEVICE)

    optimizer = optim.Adam(net.parameters(), lr =  0.000162041225040618)
    EPOCHS = 609
    losses = []
